...
png(file = "figures/nefig.png", width=800, height=800)
...
We start with the hair snag locations `CStraps` and a SpatialPolygonsDataFrame object `BLKS_BC` representing the large lakes (@fig-nefig a). Buffering 30 km around the detectors gives a naive mask (@fig-nefig b); we use a 2-km pixel size and reject points centred in a lake. The shortest dry path from many points on the naive mask to the nearest detector is much longer than the straight line distance.
```{r}
#| label: fig-static1
#| eval: true
#| cache: true
...
#|   30-km buffer around hair snags, (c) Example map of dry-path distances from an arbitrary
#|   point, and (d) Efficient mask rejecting dry-path distances >30km.
...
#| echo: false
#| cache: true
png(file = "figures/surface4a.png", width=700, height=400)
