...
For convenience we formulate the detection process in terms of hazard $\lambda(d_k(\mathbf x))$ rather than probability $g(d_k(\mathbf x))$, but the two are interchangeable[^footnoteA11a]. The overall detection rate on occasion $s$ of an AC at location $\mathbf x$ is 
\begin{equation}
 \Lambda_s(\mathbf x) = \sum_K \lambda(d_k(\mathbf x)),
\end{equation}
and aggregating over occasions gives $\Lambda(\mathbf x) = \sum_s \Lambda_s(\mathbf x)$.

[^footnoteA11a]: $\lambda(d_k(\mathbf x)) = -\log[1 - g(d_k(\mathbf x))]$.
