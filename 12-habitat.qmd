...
A mask object may also be exported as a SpatRaster using the `rast` method defined in **secr** for mask objects. See [here](11-density-model.qmd#scaleofeffect) for an example in which **terra** functions are used to smooth a mask covariate before returning it to the mask.
