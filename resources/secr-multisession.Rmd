---
title: "Multi-session models in **secr** 4.6"
...
```{r startup, echo = FALSE, message = FALSE, warning = FALSE, results = 'hide'}
library(secr)
...
for (sess in 1:3) 
    covariates(msCH[[sess]]) <- data.frame(sex = sample(c('f','m'), nrow(msCH[[sess]]),
                                                      replace = TRUE))
write.capthist(msCH, filestem = 'msCH', tonumeric = FALSE, covariates = TRUE)
```
...
```{r readmsCH}
msCH <- read.capthist('msCHcapt.txt', 'msCHtrap.txt', covnames = 'sex')
```
...
Then,
```{r readmsCH2}
msCH2 <- read.capthist('msCHcapt2.txt', 'msCHtrap.txt')
...
Given multi-session capthist input, `secr.fit` automatically fits a multi-session model by maximising the product of session-specific likelihoods (Efford et al. 2009). For fitting a model separately to each session see the later section on [Faster fitting...](#fasterfitting).
...
This is fine for a single overall lambda. However, if you are interested in successive estimates (session 1 to session 2, session 2 to session 3 etc.) the solution is slightly more complicated. Previous iterations of this document suggested using the 'backward difference' coding of the levels of the factor `session`, specified with the details argument 'contrasts'. A more flexible approach using the 'Dlambda' parameterization is available in **secr** $\ge 4.6.2$; it is described separately in [secr-trend.pdf].

```{r trendexample, cache = TRUE}
...
## Faster fitting of fully session-specific models  {#fasterfitting}
...
[^footnote1]: See the help for `addCovariates` regarding possible spatial data sources.
