...
Table: (\#tab:deviceexamples) Examples of device types

| Device | Type | 
|:------------|:------------------|
| Automatic camera | proximity detector | 
| DNA hair snag | proximity detector | 
| Sherman trap | single-catch trap | 
| Ugglan trap | multi-catch trap | 
| Mist net | multi-catch trap | 
...
```{r example, eval = TRUE, echo = FALSE, out.width = '60%', out.height = '60%', fig.cap = "Distance-dependent detection of uniformly distributed activity centres (open circles; filled if captured)"}
knitr::include_graphics('figures2/example.png')
```
...
```{r detectionfunctions, echo = FALSE, message = FALSE, fig.width=6, fig.height=4.5, fig.cap = "Some detection functions"}
library(secr)
...
### Link functions {#link}
\index{Link functions}
A simple SECR model has three parameters: density $D$, and the intercept $g_0$ and spatial scale $\sigma$ of the detection function. Valid values of each parameter are restricted to part of the number line (positive values for $D$ and $\sigma$, values between zero and one for $g_0$). A straightforward way to constrain estimates to valid values is to conduct maximisation of the likelihood on a transformed ('link') scale: at each evaluation the parameter value is back transformed to the natural scale. Common link functions are log (for positive values) and logit (for values between zero and one)[^footnote2c]. 

[^footnote2c]: $\mbox{logit}(p) = \log[p/(1-p)]$;  $\mbox{logit}^{-1}(y) = 1 / [1 + \exp(-y)]$
...
There are several ways to estimate the parameters of the SECR probability model, all of them computer-intensive. We focus on numerical maximisation of the log likelihood [@be08]. In outline, a function to compute the log likelihood from a vector of beta parameters is passed, along with the data, to an optimization function --- **secr** uses `nlm` from the **stats** package [@R-base] that allows various methods. Optimization is iterative. For illustration, Fig. \@ref(fig:MLE) shows the sequence of likelihood evaluations with two maximization algorithms when the parameter vector consists of only the intercept and spatial scale of detection. Optimization returns the maximized log likelihood, a vector of parameter values at the maximum, and the Hessian matrix from which the variance-covariance matrix of the estimates may be obtained.
