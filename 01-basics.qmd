...
as more and more peripheral individuals are encountered[^footnote1a].

[^footnote1a]: For a bounded population the number of observed individuals has 
a natural limit, the true population size. Even when it exists, this is not 
usually the number that would be estimated by non-spatial capture--recapture [@obwa78].
...
Inhomogeneous density may differ discretely, between habitats, or continuously 
in response to mapped covariates.
...
$\phi$ is a single value. If there are two habitats $\phi$ provides a 
value for each one. If density varies linearly with a covariate then $\phi$ 
comprises the intercept and slope.
...
Basic SECR uses sampling devices ('detectors') placed at known locations.
We need to recognise individuals whenever they are detected. At least some of the 
individuals should be detected at multiple locations. The
...
behaviour and the data they collect (@tbl-deviceexamples). The term 'detector type' 
groups devices according to the required probability model (@sec-pointdetectors).
...
(binary, Poisson, or binomial), with binary being the most common[^footnote1b].

[^footnote1b]: In the **secr** software, type 'proximity' refers specifically to 
...
animal until it is released can truly be called 'traps'.[^footnote1c] The probability
...
[^footnote1c]: Confusingly, **secr** uses 'traps' as a generic name for R objects holding
...
The preceding description assumes sampling at discrete points in space. Searches 
may also be conducted across space, yielding a possibly unique location for each 
detection. There are parallel SECR models for such data (@sec-areasearches, 
@sec-areaandtransectsearches). Searched areas are somewhat analogous to detectors, and may or may not allow only 
one detection per occasion, like true traps. Data from area searches may be easier 
to analyse if they are [discretized](A04-polygondetectors.qmd#sec-discretize) and 
treated as coming from many point detectors.
...
#| echo: false
#| out-width: 70%
#| fig-cap: Distance-dependent detection of a uniformly distributed population 
#|   (circles represent activity centres) at single-catch traps on a square grid 
#|   (squares). Lines indicate random captures on a single occasion - each detected
#|   individual (filled circle) is linked to the trap in which it was caught. 
#|   For this detector type SECR requires multiple sampling occasions.
...
pixel for use in density models. Buffer width and pixel
size are considered in @sec-habitat. The scale at which spatial covariates affect 
density is considered [separately](11-density-model.qmd#sec-scaleofeffect).
...
borne out by subsequent simulations. The final discussion of
@Gerber2015 is pertinent: they give an even-handed summary of the 
philosophical and practical issues, and suggest that "the asymptotic
argument against likelihood methods for SCR is moot."
@Palmero2023 reported that Bayesian methods
...
(see @sec-fixedN for background on $N(A)$, the population in area $A$).
...
Bayesian priors are usually chosen to be 'uninformative', and under these conditions the mode of the posterior distribution is expected to be a close match to the maximum likelihood estimate. Informative priors, perhaps based on estimates of detection parameters from past studies, are a way to make something out of very sparse datasets. Their use must be fully reported and explained.
