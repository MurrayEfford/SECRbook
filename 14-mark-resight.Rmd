...
Not all detector types and options in **secr** will work with mark--resight data. See [Limitations](#limitations) for a list of known constraints (some will be mentioned along the way). See the [step-by-step guide](#appendix1) for a quick introduction.
...
```{r markresightstartup, message = FALSE, warning = FALSE}
library(secr)
setNumThreads(7)
```

```{r readMRCH, message = FALSE, results = "hold"}
...
This example relies on the built-in `autoini` function to provide starting parameter values for the numerical maximization. However, the automatic values may not work with complex mark--resight models. It is not worth continuing if the first likelihood evaluation fails, as indicated by LogLik NaN when `trace = TRUE`. Then you should experiment with the `start` argument until you find values that work (see also [secr-troubleshooting.pdf]).
...
Conditional-likelihood models are generally not useful for mark-resight analyses. Individual covariates therefore should not be used in mark-resight models in **secr**. Other covariates (at the level of session, occasion, or detector) should work in any of the models. See the help for `secr.fit` and vignettes such as [secr-overview.pdf].

The mark--resight models in **secr** 4.6 do not allow groups (g). Finite mixtures, including 'hcov' models, are only partly implemented.
...
These limitations apply when fitting mark--resight models in **secr** 4.6:
...
The implementation of spatially explicit mark--resight models in 4.6 is still somewhat limited. It is intended that mark--resight models work across other capabilities of **secr**, particularly
...
```{r simMRCH, eval = FALSE, message = FALSE, warning = FALSE}
library(secr)
grid <- make.grid(detector = c("multi", rep("proximity",4)))
markocc(grid) <- c(1, 0, 0, 0, 0)
g0mr <- c(0.3, 0.1, 0.1, 0.1, 0.1)
...
```{r simMRCH5, message = FALSE, warning = FALSE, cache = TRUE}
library(secr)
...
```{r simMRCH6, message = FALSE, warning = FALSE}
...
[multi-session]: https://www.otago.ac.nz/density/pdfs/secr-multisession.pdf
