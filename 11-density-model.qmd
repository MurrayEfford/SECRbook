...
[^footnote11a]: It may also be specified in a user-written function supplied to
...
Use the arguments 'se.D' and 'cl.D' to request computation of the estimated standard error and/or upper and lower confidence limits for each mask point[^footnote11b]. If requested, values are saved as additional covariates of the output Dsurface (SE.0, lcl.0, and ucl.0 if there is only one group).

[^footnote11b]: Option available only for models specified in generalized linear model form with the 'model' argument of secr.fit, not for user-defined functions.
