...
Instability of the estimates can result when the likelihood surface has a local maximum. Numerical maximisation may then fail to find the true maximum from a given starting point. Nelder-Mead is more robust than other methods.

Finite mixture models have known problems due to multimodality of the likelihood, as discussed separately (@sec-individual-heterogeneity). Similar problems have been found in some acoustic models.

It is suspected that extreme (over-coarse) discretisation of the habitat mask sometimes results in instability due to local maxima, but this has not been proven.
