...
* the coefficients are on the link scale, and must at the very least be back-transformed to the natural scale, and
...
These complexities are handled by the `predict` method for secr objects, as for other fitted models in R (e.g., `lm`). The desired levels of covariates and other predictors appear as columns of the 'newdata' dataframe.
...
The `collate` function is a handy way to assemble a table of estimates from 
