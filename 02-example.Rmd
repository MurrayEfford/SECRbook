...
```{r esaplot, fig.width = 4.5, fig.height = 4, fig.align = "center", fig.cap = "Post hoc evaluation of buffer width using `esaPlot()` "}
par(mar = c(5,4,2,1))  # adjust white margins
esaPlot(fit)
...
hist(covariates(mask300)$pd[covariates(mask300)$d<200], breaks =
      seq(0,1,0.05), add = T, col = 'orange')
hist(covariates(mask300)$pd[covariates(mask300)$d<100], breaks =
      seq(0,1,0.05), add = T, col = 'yellow')
legend(0.75, 0.45*nm, legend = c('All mask', '100 - 200 m', 
      '< 100 m'), fill = c('forestgreen','orange','yellow'))
```
