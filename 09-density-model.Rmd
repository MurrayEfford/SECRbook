...
Plotting is easy:
```{r ovmaskplot, eval = TRUE, message=FALSE, fig.width = 6, fig.height = 5, fig.cap = "Orongorongo Valley possum study area"}
par(mar = c(1,6,2,8))
...
args <- list(capthist = OVpossumCH[[1]], mask = ovmask, trace = FALSE)
...
```{r OVsurface, eval = TRUE, fig.width = 6, fig.height = 5, fig.cap = "Quadratic possum density surface"}
par(mar = c(1,6,1,8))
...
plot(ovtrap, add = TRUE)
plot(surfaceDxy2, plottype = "contour", poly = FALSE, breaks = seq(0,22,2), 
     add = TRUE)
lines(leftbank)
```
...
### Link function
\index{Link function, density model}
...
```{r dtrplot, eval = TRUE, fig.width = 6, fig.height = 5, fig.cap = "Orongorongo Valley possum study: distance to river"}
par(mar = c(1,6,1,8))
...
RSfits <- list.secr.fit(model = models, constant = args, prefix = "RS")
```

Now add these to the AIC table and plot the 'AIC-best' model:

```{r OVAIC2, strip.white=TRUE}
AIC(c(fits, RSfits), criterion = "AIC")[,-c(2,5,6)]
```

```{r OVdtr, eval = TRUE, fig.width = 6, fig.height = 5, fig.cap = "Possum density vs distance to river: regression spline *k* = 3" }
tmp <- predict(RSfits$RS3, newdata = data.frame(DTR = seq(0,400,5)))
...
```{r CIplot, eval = TRUE, fig.width = 6, fig.height = 5, fig.cap = "Confidence surfaces"}
...
\index{Density model, prediction and plotting}
...
## Potential problems
\index{Density model, troubleshooting}
...
```{r OVfxsurfaceplt, echo=TRUE, eval = TRUE, cache = TRUE, fig.width = 6, fig.height = 5, fig.cap = "Total fx surface"}
par(mar = c(1,6,1,8))
...
```{r fxsurfaceWplt, eval = TRUE, fig.width = 6, fig.height = 5, fig.cap = "Waitarere possum fx surface"}
par(mar = c(1,5,1,8))
...
```{r relativeD, cache = TRUE, warning = FALSE, fig.width = 6, fig.height = 5, fig.cap = "Orongorongo Valley relative density"}
...
fitrd1 <- secr.fit(capthist = OVpossumCH[[1]], mask = ovmask, trace = FALSE,
         model = D ~ s(DTR, k=3), details = list(relativeD = TRUE))
...
maximising the likelihood conditional on $n$. The relative model has one fewer coefficients than the absolute density model.
...
```{r relativeD2, cache = TRUE, warning = FALSE}
...
fitrd3 <- secr.fit(capthist = OVpossumCH[[1]], mask = ovmask, trace = FALSE,
         model = D ~ pd + s(DTR,k=3), details = list(relativeD = TRUE))
```

```{r relativeD3, fig.width = 6, fig.height = 5, fig.cap = "Orongorongo Valley relative density of tagged possums"}
# compare and plot
...
[OVmask]:ovmask.png
[quadsurface]:quadsurface.png
[DTR]:DTR.png
[DTR3]:DTR3.png
[CIplot]:CIplot.png
[spline]:spline.png
[fxsurface]:fxsurface.png
[fxsurfaceW]:fxsurfaceW.png
