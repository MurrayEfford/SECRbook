...
Our main criterion will be the relative bias of an estimator, abbreviated RB and defined for an estimator $\hat \theta$ of an arbitrary parameter $\theta$ as 
$$
\mathrm{RB}(\hat \theta) = \frac{\mathrm{E} (\hat \theta - \theta)}{\theta}.
$$
For SECR, the expected values $\mathrm{E} (\hat \theta - \theta)$ are invariably estimated by obtaining $\hat \theta$ for a large sample of simulated datasets.
