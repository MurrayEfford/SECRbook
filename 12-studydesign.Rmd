...
\index{Design!Data requirements}
...
```{r goldilocks3, eval = TRUE, echo = FALSE, fig.width = 8, fig.height = 3, warning = FALSE, message = FALSE, out.width = "90%", fig.cap = "(ref:goldilockscap)"}
...
## Precision and power
\index{Design!Statistical power}
...
Precision is conveniently measured by its inverse, the relative standard error (RSE) where $\mbox{RSE}(\hat \theta) = \widehat{\mbox{SE}}(\hat \theta) / {\hat \theta}$ for estimate $\hat \theta$ of parameter $\theta$. In statistics, the standard error is the square root of the sampling variance, estimated for MLE as described in Section \@ref(confidenceintervals). Wildlife papers often use 'CV' for the RSE of estimates, but this confuses relative standard error and relative standard deviation.

The question 'What precision do I need?' requires clarity on the purpose of the study. If the purpose is to compare density estimates from different times or places then the power to detect change of a given magnitude is a direct function of the initial $\mbox{RSE}(\hat D)$ (Fig. \@ref(fig:power)). Although RSE = 0.2 is often touted as adequate "for management purposes", estimates with RSE = 0.2 have low ($\le$ 50\%) power to detect a change in density of even $\pm$ 50\% at the usual $\alpha = 0.05$. If reliable estimates are vital for population management, as claimed routinely in methodological papers, then surely greater precision is required. RSE = 0.1 is a better general target.

(ref:powercap) Power of a 2-sided test ($\alpha = 0.05$) to detect change in density between two surveys as a function of effect size ((*D~2~*/*D~1~* − 1) × 100 on the x-axis) for two levels of $\mbox{RSE}(\hat D)$ in the initial survey. For initial RSE = 0.1, only changes greater than --36\% and +44\% are detected with power greater that 80\% (dashed line). Reproduced from @eb19 Fig. 1.

```{r power, eval = TRUE, echo = FALSE, fig.width=7.5, fig.height=5, out.width = '70%', fig.cap = "(ref:powercap)"}
source('figures/power.R')
```

We lack a clear guide to required RSE in other studies, for which the effect size may be ill-defined. We have observed fitted models with $\mbox{RSE}(\hat D) \gg 0.2$ to behave erratically with respect to AIC model selection, presumably because of sampling variance in the AIC values themselves.

## Components of variance {#componentsofvariance}
...
\index{Design!Pilot parameter values}
...
## Expected sample size {#Enr}
\index{Design!Sample size}
...
Simulations described on [GitHub](https://htmlpreview.github.io/?https://github.com/MurrayEfford/secr-simulations/blob/main/ARR/secr-simulations-ARR.html) show that SECR performs poorly when the extent ratio is less than 1: the relative bias and root-mean-square error of $\hat D$ increase abruptly (Fig. \@ref(fig:ARRplot)). We earlier touted the robustness of $\hat D$ to misspecification of the [detection function](#detectfn), but this desirable property evaporates when the array is small, as shown by estimates from a hazard exponential function applied to hazard half-normal data in Fig. \@ref(fig:ARRplot). 

@e11 simulated area-search data for a range of area sizes. Plotting those results with the extent ratio as the x-axis shows a similar pattern to Fig. \@ref(fig:ARRplot) (see [GitHub](https://htmlpreview.github.io/?https://github.com/MurrayEfford/secr-simulations/blob/main/ARR/secr-simulations-ARR.html)).
...
## The $n-r$ tradeoff
\index{Design!n-r tradeoff}

The performance of the SECR density estimator depends on both the number of individuals $n$ and the number of re-detections $r$. However, we cannot maximise their expected values simultaneously. E(*n*) is greatest when detectors are far apart and each detector samples a unique set of individuals (AC). For binary proximity detectors E(*r*) is greatest when detectors are clumped together and declines monotonically with spacing; for multi-catch traps, E(*r*) increases and then declines (Fig. \@ref(fig:nrmplot)).

(ref:nrmplotcap) Sample size as a function of detector spacing for two detector types. Square grid of 64 detectors operated for 10 occasions with $D = 0.5\sigma^{-2}$ and $\lambda_0 = 0.1$.

```{r nrmplot, eval = TRUE, cache = FALSE, echo = FALSE, message = FALSE, warning = FALSE, fig.width=8, fig.height=3.5, out.width = "100%", fig.cap = "(ref:nrmplotcap)"}
source('figures/nrmplot.R')
```

To maximise the sample size we seek a satisfactory compromise, an intermediate spacing of detectors that yields both high E(*n*) and high E(*r*). There is so far no evidence for weighting one more than the other, so at the optimum E(*n*) $\approx$ E(*r*), as indicated by the vertical lines in Fig. \@ref(fig:nrmplot). The number of movement recaptures $m$ follows the pattern of total recaptures $r$ and need not be considered separately.

## Spacing and precision {#Spacing}
\index{Design!Optimal spacing}

We expect precision to improve with sample size. From the last section we understand that sample size depends somewhat subtly on detector spacing: wider spacing increases $n$ because a larger area is sampled, but it ultimately reduces $r$. We next use simulation to demonstrate the effect on the precision. We define precision as the relative standard error (RSE) of density estimates, $\mbox{RSE} (\hat D) = \widehat {\mbox{SE}} (\hat D) / \hat D$. In the following examples we assume a Poisson distribution for $n$ rather than the binomial distribution that results when $N(A)$ is [fixed](#fixedN).

<!-- (ref:spacingplotcap) Effect of detector spacing on precision of SECR density estimates. Simulations ($N = 10$); base scenario as in Fig. \@ref(fig:nrmplot). RSE within the shaded range of spacings is no more than 10\% greater then the minimum. -->

<!-- ```{r spacingplot, eval = TRUE, cache = TRUE, echo = FALSE, message = FALSE, warning = FALSE, fig.width=5, fig.height=4, out.width = "70%", fig.cap = "(ref:spacingplotcap)"} -->
<!-- source('figures/spacingplot.R') -->
<!-- ``` -->

(ref:spacingplot2cap) Relationship between precision and detector spacing for scenarios of differing grid size and detection rate $\lambda_0$ [5 sampling occasions, reproduced from Fig. 2, @eb19]. 

```{r spacingplot2, eval = TRUE, cache = FALSE, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 8, fig.height = 3.5, out.width = "95%", fig.cap = "(ref:spacingplot2cap)", results = "hide"}
source('figures/EB2019.R')
```

The simulations in Fig. \@ref(fig:spacingplot2) are representative: there is an optimal spacing (often around $2\sigma$ for a hazard half-normal detection function) and a broad range of spacings yielding similar precision. The optimum is close, but not identical, to the one that gives E(*n*) = E(*r*) in Fig. \@ref(fig:nrmplot). The variability of the simulations increases away from the optimum spacing: if we are in the right ballpark then very few replicates are needed to capture it.

Although $2\sigma$ is often mentioned as an optimum, the optimal spacing is smaller when sampling intensity is low. This happens when there are few sampling occasions or $\lambda_0$ is small, as illustrated in Fig. \@ref(fig:spacingplot2).

(ref:spacingvocccap) Effect of sampling intensity (number of occasions and $\lambda_0$) on optimal detector spacing from simulations. Base scenario as in Fig. \@ref(fig:nrmplot).

```{r spacingvocc, eval = TRUE, cache = FALSE, echo = FALSE, message = FALSE, warning = FALSE, fig.width=5, fig.height=5, out.width = "60%", fig.cap = "(ref:spacingvocccap)"}
source('figures/spacingvocc.R')
```

The curves in Fig. \@ref(fig:spacingvocc) are given by $R_{opt} = 2 \sqrt{\lambda_0 S}$ where $R_{opt}$ is the optimal spacing in $\sigma$ units and $S$ is the number of occasions. The reason for this apparently tight relationship has yet to be determined, and it is unreliable for intensive sampling (large $S\lambda_0$), as indicated by the simulations for $\lambda_0 = 0.2$ and $S \ge 10$.

<!-- What if noccasions larger? lambda_0 * noccasions-->

## Detectors required for uniform array {#Amax}

Increasing imprecision of $\hat D$ and failure of estimation in many cases (indicated by white crosses in Fig. \@ref(fig:spacingplot2)) place a limit on the size of region $A$ that can be sampled with a uniform grid of $K$ detectors. At the optimum spacing (i.e. about $2\sigma\sqrt{\lambda_0 S}$) we require $K >  A / (4\sigma^2\lambda_0 S)$. 
...
```{r verify Amax, echo=FALSE, eval = FALSE}
...
The spacing may be stretched somewhat from the optimum if we are willing to accept suboptimal precision, but increasing spacing by more than 50\% also runs the risk of bias (Fig. \@ref(fig:spacingplot2)).
...
(ref:Tip) Data from a large, clustered design may often be analysed in **secr** more quickly if the 'capthist' object is first collapsed with function `mash` into one using the geometry of a single cluster. The object retains a memory of the number of individuals from each original cluster in the attribute 'n.mash'. Functions `derived`, `derivedMash` and the method `predict.secr` use 'n.mash' to adjust their output density, SE, and confidence limits.

### Systematic clustered designs

#### Subgrids {#subgrids .unnumbered}

Random systematic designs provide a representative sample with lower sampling variance than simple random samples (REFERENCE REQUIRED). Any systematic design should have a random origin. For SECR each point on the systematic grid is the location of a subgrid. Subgrids may be any shape. We focus on square and hollow grids (Fig. \@ref(fig:cluster3x3)), but circles, hexagons, and straight lines are also possible.

If subgrids are far apart then few individuals will be caught on more than one subgrid, and they can be designed and analysed as independent units [e.g., @c19]. For design this means that the spacing of detectors on each subgrid should be optimised according to the considerations already raised in Sections \@ref(Enr) to \@ref(Spacing). For analysis it means that a homogeneous density model may be fitted to aggregated ('mashed') data from $j$ subgrids as if they were collected from a single subgrid with the shared geometry and $j$ times the density.

(ref:cluster3x3cap) Examples of systematic clustered layouts with (a) 98 detectors at 12-m spacing, and (b) 99 detectors at 10-m spacing, in a 10-ha region of interest (grey). 

```{r cluster3x3, eval = TRUE, cache = FALSE, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 8, fig.height = 4, out.width = "80%", fig.cap = "(ref:cluster3x3cap)", results = "hide"}
...
#### Lacework {#lacework .unnumbered}
\index{Design!Lacework}

In a lacework design, detectors are placed along two sets of equally spaced lines that cross at right angles to form a lattice (Fig. \@ref(fig:lace)). By making the spacing of the lattice lines (lattice spacing $a$) an integer multiple of the spacing of detectors along lines (detector spacing $b$) we avoid odd spacing at the intersections. The expected number of points on a lacework design is given by E(*K*) = *A/a^2^ (2a/b - 1)*.

(ref:lacecap) Lacework with 102 detectors at 17.4-m spacing in a 10-ha region of interest. 

```{r lace, eval = TRUE, cache = FALSE, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 5, fig.height = 5, out.width = "40%", fig.cap = "(ref:lacecap)", results = "hide"}
...
```{r laceworkspacing, echo = FALSE, eval = FALSE}
...
(ref:srsgrtscap) Non-systematic clustered layouts (a) simple random sample, and (b) Generalized Random Tessellation Stratified sample, 100 detectors in a 10-ha region of interest (grey). 

```{r srsgrts, eval = TRUE, cache = FALSE, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 8, fig.height = 4, out.width = "80%", fig.cap = "(ref:srsgrtscap)", results = "hide"}
source('figures/srsgrts.R')
```

#### Algorithmic optimisation {#algopt .unnumbered}
\index{Design!Algorithmic optimisation}
\index{Design!Genetic algorithm}

Choosing a set of detector locations may be reduced to finding a subset of potential locations that maximises some criterion (benefit function). A genetic algorithm is a robust way to search the vast set of possible layouts, and the R implementation of @w15 was applied by @drns21 and @dbss21 to SECR. The set of potential locations may simply be a fine grid of points, excluding points that are deemed inaccessible or fall in non-habitat. Results depend on the choice of criterion: @dbss21 used the minimum of E(*n*) and E(*n*), following the suggestion of @eb19 that this often leads to near-maximal precision of $\hat D$. Optimisation may take many iterations and results are not unique.

A larger criticism is that the resulting layouts are not spatially representative. The example in Fig. \@ref(fig:algopt) shows 100 detectors placed "optimally" when *R~K~* = 0.16. Detectors are clumped to maximise the precision criterion min(E(*n*), E(*r*)) without regard to spatial balance. We view this to be a major weakness and discourage general use of the method except for exploratory purposes.
...
(ref:algoptcap) Algorithmic optimisation of layout. (a) 2498 potential detector locations in a 100-ha (1-km^2^) region, and (b) Layout comprising a subset of 100 points that maximises min(E(*n*), E(*r*)) after 1000 iterations with the base parameters in Fig. \@ref(fig:nrmplot). 

```{r algopt, eval = TRUE, cache = FALSE, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 8, fig.height = 4, out.width = "80%", fig.cap = "(ref:algoptcap)", results = "hide"}
source('figures/algopt.R')
```

<!-- This is not necessarily beneficial: estimates from a regular array with near-optimal spacing (Fig. \@ref(fig:spacingvocc)) are usually more precise (e.g., @drns21 Appendix S5). However, for a large region, algorithmic optimisation provides an automated, hands-off alternative to deliberate (geometrical) clustering of detectors (1).  -->

### Comparison of clustered layouts

There is no general way to navigate the multiplicity of clustered designs. As an example, we compare the preceding clustered designs for a particular scenario: 100 binary proximity detectors in a 1-km^2^ region of interest for a species with  $\sigma = 0.02$ km (*R~K~* = 0.16) keeping other parameters as in Fig. \@ref(fig:nrmplot).

(ref:tourplotcap) Layouts with about 100 detectors in a region 1-km^2^. Colour codes the probability of detection for an individual centred in each pixel (dark green <0.05, white >0.95). Detectors are joined by a minimum-length route.

```{r tourplot, eval = TRUE, cache = FALSE, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 9, fig.height = 5, out.width = "90%", fig.cap = "(ref:tourplotcap)", results = "hide"}
source('figures/tourplot.R')
```

As before, we expected precision to be driven by sample size, $n$ and $r$, with E(*n*) maximised by scattering detectors widely. Differences among the designs with respect to the usual measures of performance (RB, RSE, rRMSE) were mostly minor (Table \@ref(tab:designcomparison)). The exceptions were SRS ($\mbox{RB}(\hat D)$ +4\%) and GRTS ($\mbox{RB}(\hat D)$ +5\%); these designs detected many individuals, but generated few re-detections. The lacework design gave similar results to SRS in this instance, but results can be improved with closer spacing along fewer lines.

The algorithmically optimised design was one of several with $\mbox{RSE}(\hat D) \approx 11\%$. Tight clustering of detectors resulted in the shortest travel distance of the designs considered. However, 'GA optimised' clearly lacked spatial balance and is therefore likely to provide biased estimates of average density if density varies systematically across the region.
...
Table: (\#tab:designcomparison) Performance of seven sampling designs (*K* $\approx$ 100 detectors) with respect to sample size (expected numbers of individuals E(*n*), re-detections E(*r*), and movements E(*m*)), relative bias RB, relative standard error RSE, root-mean-square error rRMSE, and route length *L*. RB, RSE and rRMSE are percentages based on 100 simulations for each design.
...
For the preceding comparison we fixed the number of detectors in advance. More realistically, we might set a target $\mbox{RSE}(\hat D)$ and ask how many clusters (i.e. subgrids) are required. @eb19 observed that if  clusters are independent (widely spaced) then E(*n*) and E(*r*) both scale with the number of clusters *c*, and $\mbox{RSE}(\hat D)$ scales with $1/ \sqrt c$. Thus we can predict the precision for a single cluster and extrapolate to assemblages of clusters (Fig. \@ref(fig:clusternumber)). Having determined the required number of clusters we can decide on a systematic spacing or apply the [spatial coverage](#spcosa) method.

(ref:clusternumbercap) Overall $\mbox{RSE}(\hat D)$ from cluster assemblages varying in size and single-cluster RSE.

```{r clusternumber, eval = TRUE, cache = FALSE, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 5, fig.height = 4, out.width = "60%", fig.cap = "(ref:clusternumbercap)", results = "hide"}
source('figures/clusternumber.R')
```

## Designing for spatial variation in parameters
\index{Design!Spatial variation}
...
We illustrate six scenarios for the interaction between spatial variation in density and effort, and between density and detection in Fig. \@ref(fig:spatialeffort). The estimate of overall density is unbiased when effort and detection are homogeneous (Fig. \@ref(fig:spatialeffort) a,d). Uncorrelated variation in effort does not bias $\hat D$ (Fig. \@ref(fig:spatialeffort) b). Selectively reducing effort in low-density areas (Fig. \@ref(fig:spatialeffort) c) causes significant bias, although this may be eliminated by including an area effect in the density model. 

Variation in detection parameters causes bias even when not correlated with density (Fig. \@ref(fig:spatialeffort) e). This is the effect of [inhomogeneous detection](#homogeneityofdetectors) noted previously. The effect is large when density and detection are correlated (Fig. \@ref(fig:spatialeffort) f and @McLellan2023).

(ref:spatialeffortcap) Effect of spatially varying effort (grid size 6 x 6 vs 8 x 8) and detection ($\lambda_0$ orange low vs red high) on estimate of overall density when density of AC (white dots) varies spatially between low (light green) and high (light blue). See [GitHub](https://htmlpreview.github.io/?https://github.com/MurrayEfford/secr-simulations/blob/main/STR/secr-simulations-STR.html) for simulation results.

```{r spatialeffort, eval = TRUE, cache = FALSE, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 9, fig.height = 4.5, out.width = "100%", fig.cap = "(ref:spatialeffortcap)", results = "hide"}
source('figures/spatialeffort.R')
```

Spatially representative sampling aims to avoid correlation between density and effort (Fig. \@ref(fig:spatialeffort) c). A strong spatial model for density overcomes the problem, but it cannot usually be guaranteed that suitable covariates will be found, and representative sampling insures against failure.

### Stratified sampling {.unnumbered}
\index{Design!Stratified sampling}
...
Then an estimate of overall density is $\hat D = \sum_h W_h \hat D_h$ and an estimate of the sampling variance is $\widehat {\mbox{var}} (\hat D) = \sum_{h=1}^H W_h^2 \widehat {\mbox{var}} (D_h)$.
...
(ref:stratifiedeffortcap) Design for evaluation of stratified effort.

```{r stratifiedeffort, eval = TRUE, echo = FALSE, fig.width=8, fig.height=2.5, out.width = '100%', fig.cap = "(ref:stratifiedeffortcap)"}
source('figures/stratifiedeffort.R')
```

(ref:stratifiedRSEcap) Precision of overall density estimated with stratified effort. Grey line indicates minimum RSE when 88 of 128 detectors were placed in the high-density stratum (4 occasions, 0.129; 10 occasions, 0.069).

```{r stratifiedRSE, eval = TRUE, echo = FALSE, fig.width=7, fig.height=5.5, out.width = '70%', fig.cap = "(ref:stratifiedRSEcap)"}
...
It should be stressed that our simulation results (e.g., Figs. \@ref(fig:ARRplot), \@ref(fig:spacingplot2), \@ref(fig:spacingvocc), \@ref(fig:stratifiedRSE), and Table \@ref(tab:designcomparison)) relate to particular scenarios, and the conclusions we draw may be reversed in other scenarios.
