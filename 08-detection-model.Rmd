...
```{r setupdf, echo = FALSE, results = "hide", warning = FALSE, message = FALSE}
hareCH6 <- read.capthist("data/hareCH6capt.txt", "data/hareCH6trap.txt", detector = "single")
```

```{r choicedf, warning = FALSE, cache = TRUE, message = FALSE}
...
```{r choicedfplot, echo = FALSE, fig.width = 4.5, fig.height = 5, fig.cap = "Four detection functions fitted to snowshoe hare data"}
cols <- c('red','orange','blue','forestgreen')
par(cex = 0.85, pty = 's')
plot(0,0,type='n',xlab='Distance (m)', ylab = 'Detection probability', xlim = c(0,150), ylim = c(0,0.4))
...
```{r choicedfAIC, warning = FALSE, message = FALSE, collapse = TRUE}
AIC(fits, criterion = 'AIC')[, c(2,3,4,7,8)]
```

```{r choicedfparmplot, echo = FALSE, fig.width = 6.3, fig.height = 2.6, fig.cap = "Parameter estimates from four detection functions (95% CI)."}
...
par(mfrow = c(1,3), las = 1, mar = c(4,4,2,1))
...
```{r HVPvaryz, echo = FALSE, warning = FALSE, cache = TRUE, message = FALSE}
z <- c(0.5,1,1.5,2)
...
Fitting the HVP function with $z$ fixed to different values is another way to examine the effect of shape (Fig. \@ref(fig:HVPvaryzplot)). Density estimates ranged only from `r HVPest[4]` to `r HVPest[1]`.

```{r HVPvaryzplot, echo = FALSE, fig.width = 4.5, fig.height = 5, fig.cap="HVP function fitted to snowshoe hare data with $z$ parameter fixed at four different values", results = "hide"}
par(cex = 0.85, pty = 's')
...
```{r ellipseHVP, echo = FALSE, fig.width=6, fig.height=3.5, fig.cap = "Confidence ellipse for HVP detection parameters plotted on the link scale and back-transformed to natural scale. '+' indicates MLE"}
...
\index{Detection !time-varying covariates}
...
### Regression splines
\index{Detection !regression splines}
...
\index{Detection ! behavioral response}
...
```{r behaviour, cache = TRUE}
cmod <- paste0('g0~', c('1','b','B','bk','Bk'))
# convert each character string to a formula and fit the models
fits <- list.secr.fit(model = sapply(cmod, formula), constant = list(capthist = 
    'deermouse.WSG', trace = FALSE, buffer = 80), names = cmod)
...
```{r behaviour4, echo = FALSE, eval = TRUE, cache = TRUE, results = "hide"}
...
```{r effortfit, cache = TRUE, warning = FALSE, strip.white = TRUE}
...
```{r effortfitt, cache = TRUE, strip.white=TRUE}
# shared arguments for model fits 1-4
timedf <- data.frame(tfactor = factor(c(1,2,2,3,3)), tnumeric = c(1,2,2,3,3))
args <- list(capthist = simCH, buffer = 100, biasLimit = NA, 
        timecov = timedf, trace = FALSE)
models <- c(g0 ~ 1, g0 ~ 1, g0 ~ tfactor, g0 ~ tnumeric)
details <- rep(list(list(ignoreusage = TRUE, fastproximity = FALSE)), 4)
details[[2]]$ignoreusage <- FALSE

# review arguments
data.frame(model = format(models), 
           ignoreusage = sapply(details, '[[', 'ignoreusage'))

# fit
fits <- list.secr.fit(model = models, details = details, constant = args,
                      names = c('null','usage','tfactor','tnumeric'))
AIC(fits, criterion = 'AIC')[,-c(2,5,6)]   # omit columns to save space
```
