...
To reproduce examples in the book you will need a recent version of **secr** (4.6.6 or later). Text in `this font` refers to R objects that are documented in online help for the **secr** package, or in base R. R code often generates warnings. These may be merely be reminders e.g., that a key argument has been set to a default. For clarity, we do not display routine warnings for examples in the text.
...
```{r schematic, eval = TRUE, echo = FALSE, out.width = '80%', fig.cap = '(ref:esscap)'}
