...
Table: (\#tab:vignettes) Vignettes for **secr**, excluding any already in this book.
...
Models are defined symbolically in **secr** using R formula notation. A separate linear predictor is used for each core parameter.  The core parameters are 'real' parameters in the terminology of MARK [@cw]. Three real parameters are commonly modelled in **secr**; these are denoted 'D' (for density), 'g0' (or 'lambda0' for 'hazard' models) and 'sigma'.
