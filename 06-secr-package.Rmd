...
Specifying the buffer width is an alternative to specifying a habitat mask. The choice of [buffer width](#bufferwidth) is discussed at length in Chapter \@ref(Habitat).
...
The 'details' component 'fixedbeta' provides control at a finer level: it may be used to fix certain coefficients while allowing others to vary. It is a vector of values, one for each coefficient *in the order they appear in the model*. Coefficients that are not fixed appear as NA. Check the order by applying `coef` to a fitted model, or by starting to fit a model with `trace = TRUE`. 
