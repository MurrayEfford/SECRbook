...
The parametric bootstrap is implemented in [`secr.test`] and the @Choo2024 emulation of Bayesian p-values is implemented in [`MCgof`].  We have been unable to find a 
convincing application of goodness-of-fit to SECR data. Rather than promote either method we prefer to await developments.

## Model re-fit {#sec-modelrefit}      

\index{secr.refit}

The fitting of a part-fitted model saved to a progress file may be resumed with 
[`secr.refit`] (@sec-saveprogress). The function may also be used to re-fit 
...
[`secr.test`]: https://www.otago.ac.nz/density/html/secr.test.html
[`MCgof`]: https://www.otago.ac.nz/density/html/MCgof.html
