...
of $\theta$ (see [Conditional likelihood](#conditional)).
...
with dimensions $S$ (occasions) and $K$ (detectors). Each element may be
binary (0/1) or integer (0, 1, 2, ...). Various probability models exist
for the entries $\omega_{isk}$. The appropriate probability model
...
searches](#areasearches) are considered later.
...
Binomial counts arise when there is a finite number of opportunities for
detection within each occasion that we denote $B_s$. This commonly
happens when binary proximity data are collapsed to a single occasion:
...
A single-catch trap can only catch one animal at a time. This entails
