...
## How reliable is SECR?

SECR has become popular for the benefits noted in the [Preface](#why). These do not ensure that results are trustworthy. That is a more subtle question depending on the adequacy of particular field data and the performance of the methods when assumptions are not strictly met. We briefly review some published assessments of SECR before embarking on a systematic treatment of the assumptions.

Critical evaluations of SECR have relied overwhelmingly on simulation. Empirical validation has seldom been attempted [@ewcb05; @Gerber2015; @DespresEinspenner2017].

<!-- Not everyone has fallen over themselves to endorse the robustness of SECR density estimates. Criterion of 'plausibility' (!) Right to reject implausible estimates. -->
...
@DespresEinspenner2017 chimpanzees empirical

These results are reassuring. However, the evidence from empirical validation studies is inevitably weak, owing to the wide confidence intervals and methodological constraints. For example, in the brushtail possum study, the live-trapping data were compromised by tag loss (documented in **secr**), the landward boundary of the removal area was somewhat arbitrary, and sampling may have been inadequate to represent density variation across the peninsula.

One major evaluation has questioned the reliability of SECR. @Gerber2015 re-analysed live-trapping data for rodents of several taxa confined to pens in New Mexico. The main study was followed by intensive removal using live traps to obtain the absolute density of each taxon. SECR provided instances of both over- and under-estimation, and its performance was "sometimes underwhelming". The authors focussed on heterogeneity of detection and asymmetry of home ranges as possible explanations, although direct evidence is lacking. Traps did not sample the full extent of each pen; perfect correspondence with the (pen-wide) exhaustive trapping is not to be expected if there is random or systematic variation in density of the peripheral zone. Density models with a distance-to-wall covariate provided better fit than a null model in 50\% of cases (unpubl. results).
...
### Simulation (consider in later sections)
...
Pitfalls of selective sampling? 
...
<!-- We saw this with the choice of [detection function](#detectfn) in Chapter \@ref(Detection) Fig. \@ref(fig:choicedfparmplot).  -->
...
(ref:Mtcap) Relative bias of parameter estimates from null model given temporal variation in $\lambda_0$ or $\sigma$.
