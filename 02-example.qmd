...
The last three items are generated by the `coef`, `vcov` and `predict` methods respectively. The final table of estimates is the most interesting, but it is derived from the other two. For our simple model there is one beta parameter for each real parameter[^footnote2a]. The estimated density is `r round(predict(fit)['D','estimate'],2)` hares per hectare, 95% confidence interval `r paste(round(predict(fit)['D',c('lcl','ucl')],2), collapse="--")` hares per hectare[^footnote2b].

[^footnote2a]: We can get from beta parameter estimates to real parameter estimates by applying the inverse of the link function e.g. $\hat D = \exp(\hat \beta_D)$, and similarly for confidence limits; standard errors require a delta-method approximation (Lebreton et al. 1992).

[^footnote2b]: One hectare (ha) is 10000 m^2^ or 0.01 km^2^.
...
Buffer width is not critical as long as it is wide enough that animals at the edge have effectively zero chance of appearing in our sample, so that increasing the buffer has negligible effect on estimates. For half-normal detection (the default) a buffer of $4\sigma$ is usually enough[^footnote2c]. We check the present model with the function `esaPlot`. The estimated density[^footnote2d] has easily reached a plateau at the chosen buffer width (dashed red line):

[^footnote2c]: This is not just the tail probability of a normal deviate; think about how the probability of an individual being detected at least once changes with (i) the duration of sampling (ii) the density of detector array.

[^footnote2d]: These are Horvitz-Thompson-like estimates of density obtained by dividing the observed number of individuals $n$ by [effective sampling areas](05-theory-special-topics.qmd#sec-esa) (Borchers and Efford 2008) computed as the cumulative sum over mask cells ordered by distance from the traps. The algorithm treats the detection parameters as known and fixed.
