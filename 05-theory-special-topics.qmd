...
Conditioning on $n$ *without* requiring uniform density as in the previous section leads to another result that is sometimes useful. We may be concerned to estimate relative density (the distribution of AC in relation to habitat covariates) rather than absolute density. Then we can discard the first factor in @eq-L and maximize a likelihood based on the second factor alone
$$
L_r(\phi^\prime, \theta | \Omega) \propto \prod_{i=1}^n \int_{R^2} \mathrm{Pr}(\omega_i | \mathbf x; \theta) \, f(\mathbf x ; \phi^\prime, \theta) \; d\mathbf x.
$$ {#eq-rd}
For a log link the new parameter vector $\phi^\prime$ corresponds to the original $\phi$ with one coefficient, the intercept, fixed at an arbitrary value such as zero. 

<!-- Curiously, this also seems to apply when D~x... If there are no density covariates in the model (i.e. density is assumed uniform) then  -->

This approach makes the same sampling assumptions as the full model, including that individuals enter the sample by a spatial detection process whose parameter vector $\theta$ we estimate from the data. Estimates of detection parameters by maximizing @eq-rd are in fact identical to those of the previous section.

However, in other scenarios, the *only* individuals at risk of detection may be those marked in an earlier phase of the study. This is the case for the automated detection of fish with implanted acoustic telemetry tags or passive integrated transponders (PIT) [@Whoriskey2019]. These data resemble mark--resight data (@sec-mark-resight) except that they do not include counts of unmarked animals. Estimates of $\phi^\prime$ using @eq-rd then describe only the distribution of marked individuals. Even if we can assume that AC are stationary between the two phases, $\phi^\prime$ is biased as a model for population distribution as it incorporates the spatial selectivity of marking.

Lack of information on the marking phase also afflicts mark-resight analyses [e.g., @eh18]. The possible 'solutions' are all ad hoc. We can assume that the probability of becoming marked was independent of location, or we can substitute an externally computed variable for the spatial probability (cf $p_\cdot(\mathbf x|\theta^\prime)$ where $\theta^\prime$ applies to the marking phase). Further work is expected in this area.
