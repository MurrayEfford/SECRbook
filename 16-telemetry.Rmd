...
plot(traps(trCH), border = 150, bty = 'o') # base plot
plot(combinedCH, title = 'Telemetry', type = 'telemetry', tracks = TRUE, 
     add = TRUE)
```
...
fit0 <- secr.fit(CHt, buffer = 300, detectfn = 'HHN', trace = FALSE)
predict(fit0)
```
...
trCH <- sim.capthist(tr,  popn = pop1, detectfn = "HHN", detectpar = 
                         list(lambda0 = 0.1, sigma = 25))
teCH <- sim.capthist(te,  popn = pop2, detectfn = "HHN", detectpar = 
...
## simulate telemetry
teCHD <- sim.capthist(te, popn = pop3Dt, renumber = FALSE, detectfn = "HHN",
...
trCH <- sim.capthist(tr,  popn = pop4, renumber = FALSE, detectfn = "HHN",
                     detectpar = list(lambda0 = 0.1, sigma = 25))
teCHC <- sim.capthist(te, popn = pop4C, renumber = FALSE, detectfn = "HHN",
                     detectpar = list(sigma = 25), noccasions = 1, exactN = 25)
...
te <- make.telemetry()
teCH2 <- sim.capthist(te, popn = list(D = 2, buffer = 200), detectfn = "HHN", 
  exactN = 100, detectpar = list(sigma = 25), noccasions = 1)
mask <- make.mask(traps(teCH2), buffer = 300, type = 'trapbuffer')
# fails
...
fit1000 <- secr.fit(teCH2, mask = mask, detectfn = 'HHN', CL = TRUE, 
                    trace = FALSE, details = list(telemetryscale = 1e3)) 
predict(fit1000)
```
...
Telemetry data may not be combined with mark-resight in **secr** 4.6 except possibly in distinct sessions (this has not been tested).
...
[schematicfigure]: schematic2.pdf
